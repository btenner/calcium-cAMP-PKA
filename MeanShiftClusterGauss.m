function [clustCent,data2cluster,cluster2dataCell] = MeanShiftClusterGauss(dataPts,bandWidth,plotFlag);
...
while numInitPts
    ...
    while 1     %loop untill convergence
        ...
        GausKern =  exp(-sqDistToAll/(2*bandWidth.^2));
%         inInds      = find(sqDistToAll < bandSq);               %points within bandWidth
        inInds      = find(GausKern>=exp(-numSigCutOff.^2/2));  % points within numSigCutOff number of sigma of the gaussian
        thisClusterVotes(inInds) = thisClusterVotes(inInds)+1;  %add a vote for all the in points belonging to this cluster
        
        
        myOldMean   = myMean;                                   %save the old mean
%         myMean      = mean(dataPts(:,inInds),2);                %compute the new mean
        myMean      = sum((ones(length(dataPts(:,1)),1)*GausKern(1,inInds)).*dataPts(:,inInds),2)/sum(GausKern(1,inInds));                %compute the new Kernel weighted mean
